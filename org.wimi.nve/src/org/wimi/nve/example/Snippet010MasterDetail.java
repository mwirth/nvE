...
/**
 * Snippet that displays a simple master detail use case. A list of persons is displayed in a list and upon selection the name of
 * the selected person will be displayed in a Text widget.
 */
	...
	public static void main(String[] args)
	{
		...
		Realm.runWithDefault(SWTObservables.getRealm(display), new Runnable()
		{
			public void run()
			{
				...
				IObservableValue detailObservable = BeansObservables.observeDetailValue(selection, "name", String.class);

				// 3. Bind the Text widget to the name detail (selection's
				// name).
				new DataBindingContext().bindValue(SWTObservables.observeText(name, SWT.None), detailObservable,
